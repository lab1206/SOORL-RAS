...
STEP_TIMEOUT_LENGTH = (
    30  # how long in second(s) a step can take before a timeout error is triggered.
)


class MAbUpstreamMPC:
    def __init__(
        self, controller, action_dim=7 + 1 + 1, observation_dim=17 + 2 + 1951
    ) -> None:
        ...
        self.mpc_cont = controller._build_mpc_up(
            Q,
            R,
            N,
            controller.dt_spl,
            controller.xss[: controller.Nx_up],
...
class MAbUpstreamEMPC:
    def __init__(
        ...
        self.empc_cont = controller._build_empc_up(
            N,
            controller.dt_spl,
            controller.xss[: controller.Nx_up],
            controller.uss[: controller.Nu_up],
...
class MAbEnvGym(smplEnvBase):
    def __init__(
        self,
        dataset_dir="smpl/configdata/mabenv",
        dense_reward=True,
        normalize=True,
        debug_mode=False,
        action_dim=7 + 1 + 1,
        observation_dim=17 + 2 + 1951,
        ...
        ) = self.utils_helper.load_bounds(self.dataset_dir)
        (
            self.min_actions,
            self.max_actions,
            self.min_observations,
            ...
            self.min_actions / uscale,
            self.max_actions / uscale,
            self.min_observations / xscale,
            self.max_observations / xscale,
        )
        ...
        self.random_seed = (
            np.random.random_integers(0, 2**32 - 1)
    ...
    def observation_beyond_box(self, observation):
        ...
        return (
            np.any(
                observation[: self.upstream_states]
                > self.max_observations[: self.upstream_states] * 1.05
            )
            or np.any(
                observation[: self.upstream_states]
    ...
    def reward_function_standard(
        ...
        if reward is not None:
            return reward
        elif self.observation_beyond_box(current_observation) or self.action_beyond_box(
            action
        ):
        ...
        if self.standard_reward_style == "setpoint":
            reward = -(
                np.square(
                    current_observation[: 17 + 2] - self.steady_observations[: 17 + 2]
                )
            ).mean()
        else:
            ...
            uu = action * uscale
            productivity = (
                xx[6] * uu[1] + current_observation[14] * uu[3]
            )  # range of reward is [0, inf)
            if self.standard_reward_style == "productivity":
                ...
                downstream_yield = 1 - current_observation[-14] / (
                    current_observation[19] + 1e-8
                )  # range of downstream_yield is [0,1].
                reward = productivity * downstream_yield
            else:
                raise ValueError(
                    "standard_reward_style should be either 'setpoint' or 'productivity'"
                )
    ...
    def evenly_spread_initial_states(self, val_per_state, dump_location=None):
        ...
        for i in range(val_range):
            ...
            initial_states[i] = np.concatenate(
                [initial_states[i], self.steady_observations[self.upstream_states :]]
            )
    ...
    def _simulation(self, xk, uk):
        ...
        xkp1[-1] += self.dt_itgr * (
            xkp1[19] - xkp1[-14]
        )  # difference between inlet concentration and outlet concentration
        return xkp1
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            try:
                for i in range(0, self.controller.dt_ratio):
                    xk = self._simulation(
                        self.Xi[self.step_count * self.controller.dt_ratio + i], action
                    )
        ...
        for n_epi in tqdm(range(num_episodes)):
            ...
            if to_plt:
                ...
                for n_o in range(self.observation_dim):
                    ...
                    plt.plot(
                        [self.steady_observations[n_o] for _ in range(self.max_steps)],
                        linestyle="-.",
                    ...
                    plt.annotate(
                        str(self.steady_observations[n_o]),
                        xy=(0, self.steady_observations[n_o]),
                    )
                    plt.legend()
                    if plot_dir is not None:
                ...
                for n_a in range(self.action_dim):
                    ...
                    plt.plot(
                        [self.steady_actions[n_a] for _ in range(self.max_steps)],
                        linestyle="-.",
