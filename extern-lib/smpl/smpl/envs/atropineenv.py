...
class AtropineMPC:
    def __init__(
        ...
        Nu=4,
        uss_subtracted=True,
        reward_on_ess_subtracted=False,
        reward_on_steady=True,
        reward_on_absolute_efactor=False,
        reward_on_actions_penalty=0.0,
        reward_on_reject_actions=True,
        relaxed_max_min_actions=False,
        observation_include_t=True,
        observation_include_action=False,
        observation_include_uss=True,
        observation_include_ess=True,
        observation_include_e=True,
        observation_include_kf=True,
        observation_include_z=True,
        observation_include_x=False,
    ):
        ...
        self.relaxed_max_min_actions = (
            relaxed_max_min_actions  # assume uss_subtracted = false.
        )
    ...
    def predict(self, state):
        ...
        uk = (
            atropine_mpc_controller(
                ...
                INPUT_REFS,
                OUTPUT_REF,
                self.model_preconfig[0],
                self.model_preconfig[1],
                self.model_preconfig[2],
                self.model_preconfig[3],
            )
            / 1000
        )  # unscale
...
class AtropineEnvGym(smplEnvBase):
    def __init__(
        ...
        self.observation_dim = (
            1 * self.observation_include_t
            + 4 * self.observation_include_action
            + 4 * self.observation_include_uss
            + 1 * self.observation_include_ess
            + 1 * self.observation_include_e
            + 2 * self.observation_include_kf
            + 30 * self.observation_include_z
            + 1694 * self.observation_include_x
        )
        ...
        max_observations = []
        if self.observation_include_t:
            max_observations.append(
                np.ones(1, dtype=np.float32) * 100.0
            )  # by convention
        if self.observation_include_action:
        ...
        if self.observation_include_z:
            max_observations.append(
                np.ones(30, dtype=np.float32) * 0.5
            )  # by convention
        if self.observation_include_x:
            max_observations.append(
                np.ones(1694, dtype=np.float32) * 50.0
            )  # by convention
        try:
            self.max_observations = np.concatenate(max_observations)
        except ValueError:
            raise Exception(
                "observations must contain something! Need at least one array to concatenate"
            )
        min_observations = []
        if self.observation_include_t:
        ...
        if self.observation_include_z:
            min_observations.append(
                np.ones(30, dtype=np.float32) * 0.0
            )  # by convention
        if self.observation_include_x:
            min_observations.append(
                np.ones(1694, dtype=np.float32) * 0.0
            )  # by convention
        try:
        ...
        if not self.uss_subtracted:
            self.max_actions = np.array(
                [0.408, 0.125, 0.392, 0.214], dtype=np.float32
            )  # from dataset
            self.min_actions = np.array(
                [0.4075, 0.105, 0.387, 0.208], dtype=np.float32
            )  # from dataset
            if self.relaxed_max_min_actions:
                self.max_actions = np.array(
                    [0.5, 0.2, 0.5, 0.4], dtype=np.float32
                )  # from dataset
                self.min_actions = np.array(
                    [0.3, 0.0, 0.2, 0.1], dtype=np.float32
                )  # from dataset
        else:
            self.max_actions = np.array(
                [1.92476206e-05, 1.22118426e-02, 1.82154982e-03, 3.59729230e-04],
                dtype=np.float32,
            )  # from dataset
            self.min_actions = np.array(
                [-0.00015742, -0.00146234, -0.00021812, -0.00300454], dtype=np.float32
            )  # from dataset
            if self.relaxed_max_min_actions:
                self.max_actions = np.array(
                    [2.0e-05, 1.3e-02, 2.0e-03, 4.0e-04], dtype=np.float32
                )  # from dataset
                self.min_actions = np.array(
                    [-0.00016, -0.0015, -0.00022, -0.00301], dtype=np.float32
                )  # from dataset
        if self.normalize:
        ...
        self.yss = self.plant.calculate_Efactor(
            DM(self.z0)
        )  # steady state output, e-factor
    ...
    def sample_initial_state(self, no_sample=False, lower_bound=0.99, upper_bound=1.01):
        if no_sample:
            ...
            x0_c = self.x0.clip(
                1e-30
            )  # treat all negative concentration values as numerical error and set to close to zero to avoid numerical issues.
            low = x0_c * lower_bound
            up = x0_c * upper_bound
            x0 = np.random.uniform(low, up)
        self.zk = DM(self.z0)  # shape is (30,)
        self.xk = self.plant.mix_and_get_initial_condition(x0, USS)[
            0
        ]  # shape is (1694,)
        self.previous_efactor = (
            self.yss
        )  # steady state output, e-factor, the small the better
        observations = []
        if self.observation_include_t:
            observations.append(
                np.array([0], dtype=np.float32)
            )  # since initial state should have self.step_count = 0
        if self.observation_include_action:
    ...
    def done_calculator_standard(
        ...
        if (
            step_count >= self.max_steps and self.max_steps != -1
        ):  # same as range(0, max_steps)
            done_info["terminal"] = True
    ...
    def _step(self, action, normalize=None):
        ...
        if self.observation_include_e:
            observations.append(
                np.array([self.previous_efactor], dtype=np.float32)
            )  # !!!!!!!also, shall I run a step here? how do we align?
        if self.observation_include_kf:
            self.xe = state_estimator(
                self.xe,
                uk,
                efactor - self.yss,  # self.Xhat[k] is previous step xe
                self.model_preconfig[0],
                self.model_preconfig[1],
                self.model_preconfig[2],
                self.model_preconfig[4],
            )
            observations.append(self.xe)
        if self.observation_include_z:
            observations.append(self.zk.full().flatten())
        ...
        reward = self.reward_function(
            self.previous_observation, action, observation, reward=reward
        )
        # compute done
        if not done:
            done, done_info = self.done_calculator(
                observation, self.step_count, reward, done=done, done_info=done_info
            )
        self.previous_observation = observation
    ...
    def plot(self, show=False, efactor_fig_name=None, input_fig_name=None):
        ...
        target_inputs = (
            np.array(target_inputs) * 1000
        )  # scale the solution to micro Litres
        ...
        axs[0, 0].step(
            ...
            where="post",
            linestyle="--",
            label="Steady State Input",
        )
        axs[0, 0].set_ylabel("U1 [\u03bcL/min]")
    ...
    def evaluate_rewards_mean_std_over_episodes(
        self,
        algorithms,
        num_episodes=1,
        error_reward=None,
        ...
        """
        result_dict = {}
        observations_list, actions_list, rewards_list = self.evalute_algorithms(
            algorithms,
        ...
        warn(
            "The function evaluate_rewards_mean_std_over_episodes is deprecated. Please use report_rewards.",
            DeprecationWarning,
            stacklevel=2,
        )
        for n_algo in range(len(algorithms)):
            ...
            if computer_on_episodes:
                ...
                for n_epi in range(num_episodes):
                    if (
                        rewards_list_curr_algo[n_epi][-1] == self.error_reward
                    ):  # if error_reward is provided, self.error_reward is overwritten in self.evalute_algorithms
