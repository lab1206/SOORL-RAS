...
class ReactorMPC:
    ...
    def __init__(
        self, Nt=20, dt=0.1, Q=np.eye(3) * 0.1, R=np.eye(2) * 0.0, P=np.eye(3) * 0.1
    ):
    ...
    def build_controller(self):
        ...
        ode_casadi = mpc.getCasadiFunc(
            self.model.ode, [self.Nx, self.Nu], ["x", "u"], funcname="odef"
        )
        ...
        contargs = dict(
            ...
            guess={"x": self.xs[: self.Nx], "u": self.us[: self.Nu]},
        )
...
class ReactorMLPReinforceAgent(torch.nn.Module):
    ...
    def forward(self, obs, return_distribution=True):
        mu = self.mu_net(obs)
        std = torch.maximum(
            torch.exp(self.log_std), torch.ones_like(self.log_std) * 1e-4
        )
        ...
        if return_distribution:
            ...
            action = np.clip(
                dist.sample().numpy().flatten(), [0, 0], [100, 0.2]
            )  # clip actions
            return action
...
class ReactorEnvGym(smplEnvBase):
    def __init__(
        ...
        self.steady_observations = np.array(
            STEADY_OBSERVATIONS, dtype=self.np_dtype
        )  # cA, T, h
        ...
        # random seed
        self.random_seed = (
            random_seed if random_seed is not None else np.random.randint(0, 10000)
        )
    ...
    def reward_function_standard(
        ...
        if self.compute_diffs_on_reward:
            previous_observation_evaluated = self.evaluate_observation(
    ...
    def find_outperformances(
        self, algorithms, rewards_list, initial_states, threshold=0.05, top_k=10
    ):
        """
        ...
        for n_epi in range(num_episodes):
            ...
            average_outperformance = max(
                self.find_outperformances_compute_average_outperformances(rewards)
            )
            ...
            try:
                ...
                top_k_better.put(
                    (
                        average_outperformance + random.uniform(0, 1e-8),
                        initial_states[n_epi],
                    )
                )
    ...
    def sample_initial_state(self):
        init_observation = np.maximum(
            np.random.uniform(
                low=(1 - self.initial_state_deviation_ratio) * self.steady_observations,
                high=(1 + self.initial_state_deviation_ratio)
                * self.steady_observations,
            ),
            0,
            dtype=self.np_dtype,
        )
        init_observation = init_observation.clip(
    ...
    def evaluate_observation(self, observation):
        ...
        return float(
            -(
                np.mean(
                    (observation - self.steady_observations) ** 2
                    / np.maximum(
